...
const [refresh,setRefresh]=useState(0);
  ...
  const handleDelete=async(categoryId:string)=>{
    try {
        const { data } = await AxiosInstance.delete(`/categories-blog/${categoryId}`);
    ...
    <div className="max-w-screen-xl mx-auto px-4 md:px-8">
      <div className="items-start justify-between md:flex">
        ...
        <div className="mt-3 md:mt-0">
          <Link
            href="/admin-dashboard/blogCategories/add"
            ...
            {categoryData && categoryData.length>0 &&
              categoryData.map((item, idx) => (
                <tr key={idx}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <img src={item.categoryImage} width="30px" height="30px"></img>
                  </td>
                  <td className="text-right px-6 whitespace-nowrap">
                    <Link
                      href={`/admin-dashboard/blogCategories/view/${item.categoryId}`}
                    ...
                    </Link>
                    {
                      user && user.role=="super-admin" && ( <Link
                        href={`/admin-dashboard/blogCategories/edit/${item.categoryId}`}
