...
const [categoryData,setCategoryData]=useState<CategoryData>();
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      const { data } = await AxiosInstance.get(`/categories-deal/${categoryId}`);
  ...
  const onSubmit = async (e:any) => {
    try {
      ...
      const { data } = await AxiosInstance.patch(`/categories-deal/${categoryId}`, formData);
      console.log(data.data);
  ...
  return (
    <>
      ...
      <form onSubmit={onSubmit}>
        {categoryData && <div className="flex flex-col px-5 gap-[1rem] w-full items-center">
